...
class Player(Ship):
    ...
    def laser_render(self, layser_list):
        ...
        for i in range(len(layser_list)):
            laser = Laser(layser_list[i]['x'], layser_list[i]['y'], YELLOW_LASER)
...
enemies = []
data_dict = {'ready':False, 'level':0 ,'user1':'0.0.0.0','user2':'0.0.0.0','x1':0, 'y1':0,'heahlth1':100, 'lost1':False, 'x2':0, 'y2':0, 'health2':100, 'lost2': False}
def main(addr):
    ...
    def redraw_window():
        ...
        WIN.blit(BG, (0,0))
        # draw text
        user1 = small_font.render(f"User1: {data_dict['user1'][0]}: {data_dict['user1'][1]}", 1, (0,255,100))
        user2 = small_font.render(f"User2: {data_dict['user2'][0]}:{data_dict['user2'][1]}", 1, (0,255,100))
    ...
    while True:
        ...
        if data_dict['lost1'] or data_dict['lost2'] or data_dict['user1']=='0.0.0.0' or data_dict['user2']=='0.0.0.0':
            user_data['ready'] = False    # rest it to False to stop it to send the previous not valid ready again in this new round
        ...
        if data_dict['ready']:
            ...
            # print(len(keys))
            WIN.blit(BG, (0,0))
            waiting_label = waiting_font.render("Waiting for another player...(1 online player)", 1, (255,255,255))
            WIN.blit(waiting_label, (WIDTH/2 - waiting_label.get_width()/2, 350))
            pygame.display.update()
            win = False
            lost = False
        else:
            ...
            if lost:
                title_label = waiting_font.render("You Lost!!!, Press space to ready again", 1, (255,255,255))
            elif win:
                title_label = waiting_font.render("You Won!!!, Press space to ready again", 1, (255,255,255))
            else:
                title_label = title_font.render("Press space to ready", 1, (255,255,255))

...
def handle_client(conn, addr):
    ...
    while run:
        ...
        elif data['user1'] == '0.0.0.0' or data['user2'] == '0.0.0.0' or data['lost1'] or data['lost2']:
        ...
        if keys[(pygame.K_a)-93] and player.x - player_vel > 0: # left
            player.x -= player_vel
        if keys[pygame.K_d-93] and player.x + player_vel + player.get_width() < WIDTH: # right
            player.x += player_vel
        if keys[pygame.K_w-93] and player.y - player_vel > 0: # up
            player.y -= player_vel
        if keys[pygame.K_s-93] and player.y + player_vel + player.get_height() + 15 < HEIGHT: # down
            player.y += player_vel
        ...
        for i in range(len(enemies)):
            ...
            enemies_data.append({'ex':enemies[i].x, 'ey':enemies[i].y, 'ecolor':color})
